...
const GUIDE_JSON_LD = {
  ...
  "@type": "Article",
  headline: "Vector search API for semantic similarity and embeddings",
  url: "https://ydb-qdrant.tech/guides/vector-search-api-semantic-similarity-embeddings/",
  description:
    "How to use YDB-Qdrant as a Qdrant-compatible vector search API for embeddings, semantic similarity, RAG prototypes, and YDB-backed agent memory.",
  about: ["vector search", "semantic similarity", "embeddings", "RAG", "YDB"],
};

export default function VectorSearchApiSemanticSimilarityGuidePage() {
  return (
    <>
      <script type="application/ld+json">{JSON.stringify(GUIDE_JSON_LD)}</script>
      <AgentResourcePage
        eyebrow="Guide"
        title="Vector search API for semantic similarity and embeddings"
        lead={
          <p>
            YDB-Qdrant gives YDB-backed applications a Qdrant-compatible REST
            API for storing embedding vectors, retrieving points, and running
            exact top-k semantic similarity search.
          </p>
        }
        actions={[
          {
